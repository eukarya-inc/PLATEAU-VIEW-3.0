...
const parseDeprecatedTag = (v: string) => v.match(/^\((.+)\)$/)?.[1] || v;

const splitTag = (v: string) => v.split(".");

export const parseAttributes = async (csv: string) => {
  ...
  for (const line of lines) {
    ...
    const tag1 = splitTag(columns[2]).map(parseDeprecatedTag);
    const tag2 = splitTag(columns[3]).map(parseDeprecatedTag);
    const tag3 = splitTag(columns[4]).map(parseDeprecatedTag);
    const tag4 = splitTag(columns[5]).map(parseDeprecatedTag);
